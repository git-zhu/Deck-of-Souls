...
- **体量**：12 层 × 3 幕；30 张卡牌、18 种敌人、21 个地图事件、14 枚护符、6 种出身
...
- **姿态**：敌人姿态条打空触发崩解决策点——**处决**（大伤害，碎星将军的义肢 ×1.5）或**防反**（护甲 +1 集中）。
...
- **流派化奖励**：战后选牌按属性倾向保底（智力倾向 → 3 张中至少 1 张魔法卡）。
...
- **NG+**：通关解锁；敌人 HP +25%/级、伤害 +15%、卢恩 +30%。
- **誓约 Ⅰ–Ⅲ** 与**誓言挑战**（无瓶 / 强敌）。
- **死亡回响**：阵亡后可夺回一半卢恩，或转化为记忆；记忆 ≥100 → 开局护符三选一。
...
45 个无头测试 + 贪心 bot 平衡门，全部 `godot4.6 --headless --path . --script tools/<name>` 运行：
...
- 平衡基调：贪心 bot 满资源单挑普通战接近 100% 属预期（难度预算集中在精英/Boss 与外循环修饰）；一贫如洗 vs 玛尔基特 ≈74%。详见 `docs/superpowers/specs/2026-08-17-design-review-soulslike-direction.md`。
